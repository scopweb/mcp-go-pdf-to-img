...
## PDF Compression Solution

**Problem:** Large PDF files due to high-resolution images, embedded fonts, or unnecessary metadata.

**Common Causes:**
- High-resolution images (e.g., from Illustrator exports)
- Embedded fonts not used in the document
- Excessive metadata or annotations
- Scanned documents without optimization
- Complex vector graphics
- Multiple layers or hidden elements

**Solution Implemented:** Added `pdf_compress` tool to MCP Server using pdfcpu library.

### Features
- Automatic image compression and downsampling
- Removal of unused embedded fonts
- Metadata cleanup
- Optimization of PDF structure
- Preservation of document quality
...
```json
{
  ...
  "parameters": {
    ...
    "output_path": "compressed_document.pdf"
  }
}
```

### Expected Results
- 30-70% size reduction depending on content
- Maintained readability and quality
- Faster loading and sharing

## Files Modified

- `mcp/server.go` - Added pdf_compress tool
- `go.mod` - Added pdfcpu dependency
- `README.md` - Updated features
- `EXAMPLES.md` - Added compression examples
